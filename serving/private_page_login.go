...
func servePrivatePageLogin(w http.ResponseWriter) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.WriteHeader(http.StatusOK)
	fmt.Fprint(w, privatePageLoginHTML)
}

const privatePageLoginHTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Sign In To View This Page</title>
<style>
:root {
  color-scheme: light;
  --bg: #f3efe6;
...
<main class="card">
  ...
  <p>This app lives on its own origin. Sign in here so the hub can set a local session cookie for this page.</p>
  <p id="auto-note" class="hidden">Found a stored key for this origin. Trying to sign you in automatically.</p>
  ...
  <p class="hint">Use a separate identity on untrusted pages. This login only creates a session for the current origin.</p>
</main>
<script>
(function() {
'use strict';
...
function openDB() {
  return new Promise(function(resolve, reject) {
    var req = indexedDB.open('dv_page_auth', 1);
...
document.getElementById('btn-pass').addEventListener('click', async function() {
  ...
  try {
    ...
    await storeKey(kp.privateKey).catch(function() {});
    localStorage.setItem('dv_page_auth_username', username);
    localStorage.setItem('dv_page_auth_mode', 'pass');
...
document.getElementById('btn-pem').addEventListener('click', async function() {
  ...
  try {
    ...
    await authenticate(kp.privateKey, kp.pubkey);
    await storeKey(kp.privateKey).catch(function() {});
    localStorage.setItem('dv_page_auth_mode', 'pem');
    setStatus('Signed in. Redirecting…', 'ok');
    finish();
  } catch (err) {
    setStatus(err.message, 'error');
