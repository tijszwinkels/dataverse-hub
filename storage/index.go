...
func (idx *Index) Add(ref string, item *object.Item, ts time.Time, realms ...object.InField) {
	idx.mu.Lock()
	defer idx.mu.Unlock()
	idx.addLocked(ref, item, ts, realms...)
}
...
func (idx *Index) Update(ref string, item *object.Item, ts time.Time, realms ...object.InField) {
	idx.mu.Lock()
	defer idx.mu.Unlock()
	idx.removeLocked(ref)
	idx.addLocked(ref, item, ts, realms...)
...
func (idx *Index) GetMeta(ref string) (object.ObjectMeta, bool) {
	idx.mu.RLock()
	defer idx.mu.RUnlock()
	m, ok := idx.meta[ref]
	return m, ok
...
func (idx *Index) addLocked(ref string, item *object.Item, ts time.Time, realms ...object.InField) {
	...
	// Meta
	idx.meta[ref] = object.ObjectMeta{
		Ref:             ref,
		Pubkey:          item.Pubkey,
		Type:            item.Type,
		...
		PageRef:         pageRef,
		MimeType:        mimeType,
		IsPublic:        realm.IsPublicObject(resolved),
	...
	for relType, entries := range item.Relations {
		for _, raw := range entries {
			...
			idx.inbound[rr.Ref] = append(idx.inbound[rr.Ref], object.RelationEntry{
				SourceRef:    ref,
...
func sortMetaDesc(metas []object.ObjectMeta) {
